...
export function requireRole(session: UserSession | null, requiredRole: UserRole): UserSession {
...
export function requirePermission(session: UserSession | null, permission: string): UserSession {
  const authSession = requireAuth(session);
...
export function getRedirectPath(session: UserSession | null, requestedPath: string): string {
  if (!session || !session.isAuthenticated) {
    return '/login';
  }
